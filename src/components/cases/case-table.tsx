...
import { listCasesAction } from "@/app/(app)/cases/actions";
import {
  Table,
  TableBody,
  TableCell,
...
const columns = columnHelper.columns([
  columnHelper.accessor("title", {
    header: "Case",
    cell: (ctx) => <span className="line-clamp-1">{ctx.getValue()}</span>,
  }),
  columnHelper.accessor("severity", {
    header: "Severity",
    cell: (ctx) => (
      <Badge variant={SEVERITY_BADGE_VARIANT[ctx.getValue()]}>
        {SEVERITY_LABEL[ctx.getValue()]}
      </Badge>
    ),
  }),
...
export function CaseTable() {
  ...
  if (!data?.length) {
    return (
      <p className="py-10 text-center text-sm text-muted-foreground">
        No cases yet — run a simulation above.
      </p>
    );
  }
  ...
  return (
    <>
      ...
      <CaseDetailDialog caseId={openId} onClose={() => setOpenId(null)} />
    </>
  );
}
